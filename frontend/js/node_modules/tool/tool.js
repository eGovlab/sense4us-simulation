...
Tool.prototype.deleteSyncedModel = function(id) {
    ...
    return network.DELETE(
        ...
        that.emit('deletedModel',       removedModel, id);
        that.emit('deletedSyncedModel', removedModel, id);
...
Tool.prototype.deleteLocalModel = function(id) {
    ...
    console.log('Removing model:', removedModel);

    delete this.savedModels.local[id];
    this.emit('deletedModel',      removedModel, id);
    this.emit('deletedLocalModel', removedModel, id);
...
Tool.prototype.deleteModel = function(id) {
    var syncId = id;
    // if id is omitted, delete the currently loaded model.
    if(id === undefined) {
        if(!this.loadedModel
        || (typeof this.loadedModel.syncId !== 'number' && typeof this.loadedModel.id !== 'number')) {
            return Promise.reject(new Error('No model is loaded. This is unexpected behaviour.'));
        }

        id     = this.loadedModel.id;
        syncId = this.loadedModel.syncId;
    }

    if(!this.savedModels.synced[syncId]) {
        return this.deleteLocalModel(id);
    }

    return this.deleteSyncedModel(syncId);
};
...
Tool.prototype.saveModel = function(id) {
    ...
    if(id) {
        loadedModel = savedModels.synced[id] || savedModels.local[id];
        if(!loadedModel) {
            return Promise.reject(new Error('No model to save on id: ' + id));
        }
    }
...
Tool.prototype.loadSyncedModel = function(id) {
    if(this.savedModels.synced[id]) {
        this.loadedModel = this.savedModels.synced[id];
...
Tool.prototype.loadModel = function(id) {
    if(id === undefined) {
        ...
        if(!model) {
            ...
            return this.loadSyncedModel(model.syncId);
        }
...
Tool.prototype.notify = function(settings) {
    ...
    var options = {
        ...
        text:       settings.text       || 'No message.',
        background: settings.background || 'rgba(160, 160, 160, 1)',
        color:      settings.color      || '#fafafa'
    };

    var that         = this,
        notification = new Notification(options.text, options.background, options.color);
    that.config.notification.appendChild(notification.element);

    return new Promise(function(fulfill, reject) {
        setTimeout(function() {
            notification.fadeOut(function() {
                that.config.notification.removeChild(notification.element);
                fulfill();
            });
        }, options.delay);
    });
